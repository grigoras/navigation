...
public class DirectionsJSONParser {
	...
	public List<List<HashMap<String, String>>> parse(JSONObject jObject) {
		...
		try {
			...
			SharedVariables.directions.clear();
			SharedVariables.distances.clear();
			...
			for (int i = 0; i < jRoutes.length(); i++) {
				...
				for (int j = 0; j < jLegs.length(); j++) {
					...
					for (int k = 0; k < jSteps.length(); k++) {
						...
						String instr = (String) ((JSONObject) jSteps.get(k))
								.get("html_instructions");
						ind = ind + l;
						SharedVariables.directions.add(new Pair<String, Integer>(instr, ind));

						String dist = "";
						dist = (String) ((JSONObject) ((JSONObject) jSteps.get(k)).get("distance"))
								.get("text");
						SharedVariables.distances.add(new Pair<String, Integer>(dist, ind));
